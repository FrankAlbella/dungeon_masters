...
func look_updown_rotation(rotation = 0):
	...
	"""
	var toReturn = Yaw.get_rotation() + Vector3(rotation, 0, 0)
...
func mouse(event):
	...
	Yaw.set_rotation(look_updown_rotation(event.relative.y / -200))
...
func _input(event):
	...
	if event is InputEventMouseButton:
		if Input.is_action_just_pressed("zoom_in"):
			if translation.z > MIN_ZOOM:
				self.translate(Vector3(0, 0, -0.1))
		if Input.is_action_just_pressed("zoom_out"):
			if translation.z < MAX_ZOOM:
				self.translate(Vector3(0, 0, 0.1))
